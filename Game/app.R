      ...
      tabItem(tabName = "home",
              ...
              tags$div(style="position:relative;z-order:1",
                tabBox( width = "100%",
                  ...
                  p(tags$b("Goal"),": Get a job ASAP by increasing scores (>30)!",align='center')
                )
              )
      )
      ,
...
server <- function(input, output, session) {
  ...
  output$referenceData <- downloadHandler(
    ...
    content = function(file) {
      file.copy("www/References.docx", file)
    }
  )
  ...
  gamevals <- reactiveValues(Status=Status,Identiy=Identiy,Previous=Previous,PPrevious=PPrevious,Wrong=Wrong,Count=Count,Frozen=Frozen,
                             mole=mole,Status2=Status2,score=score,Images=Images,clicked=FALSE)
  # Timer function:
  ...
  processClickEvent2 <- function(gridrow,gridcol){
    if ((gamevals$Status2[gridrow,gridcol]==1)&&(started())){
      gamevals$Status2 <- Status2
      # gamevals$clicked <- TRUE
      gamevals$score <- gamevals$score +1 
    }
  }
  # update score
  output$Score <- renderText(paste("Score",as.character(gamevals$score)))
  

    observeEvent(input$click11,{processClickEvent2(1,1)})
    observeEvent(input$click12,{processClickEvent2(1,2)})
    observeEvent(input$click13,{processClickEvent2(1,3)})
    observeEvent(input$click14,{processClickEvent2(1,4)})
    observeEvent(input$click21,{processClickEvent2(2,1)})
    observeEvent(input$click22,{processClickEvent2(2,2)})
    observeEvent(input$click23,{processClickEvent2(2,3)})
    observeEvent(input$click24,{processClickEvent2(2,4)})
    observeEvent(input$click31,{processClickEvent2(3,1)})
    observeEvent(input$click32,{processClickEvent2(3,2)})
    observeEvent(input$click33,{processClickEvent2(3,3)})
    observeEvent(input$click34,{processClickEvent2(3,4)})
    observeEvent(input$click41,{processClickEvent2(4,1)})
    observeEvent(input$click42,{processClickEvent2(4,2)})
    observeEvent(input$click43,{processClickEvent2(4,3)})
    observeEvent(input$click44,{processClickEvent2(4,4)})
  ...
  checkFinalScore <- function(score){
    ...
    thresholdscore <- 30#max(45 - sumscore,2)
    if (sumscore >= thresholdscore && score>4){
      return("win")
    } else {
      return("lose")
    }
