# Creators
# Ian Tay Rui-Ming - Skillsworkshop & Instructions Page
# Chan Kian Quan - Career Fair  & Instructions Page
# Wu Xinyun - Apply for job & Skillsworkshop & Career Fair 
# Ang Kang Xian - Welcome Page & Avatar & Landing Page 

# PS: Use the section at the bottom left to look at the different sections of code.
...
server <- function(input, output, session) {
  ...
  #Xinyun 
  output$Sentence <- renderImage({
    out <-vals$sentence 
    if(is.null(out)){
      text <- "Welcome!"
    }
    else{
      out <- strsplit(out, "(?:\\s|(?<=[a-z]))(?=[A-Z])", perl = TRUE)[[1]]
      text <- ""
      for(i in 1:length(out)){
      text <- paste0(text,str_c(out[i],"\n"))
        }
    }
    ...
    plot <- ggplot() + 
      ...
      theme_void()
    ggsave("www/save.png",plot)
    #ggsave("www/save.png",plot,width=40,height=10)
    imgsrc = 'www/save.png'
    list(src=imgsrc,style="max-width:400px;width:100%;") 
  },deleteFile=FALSE)
